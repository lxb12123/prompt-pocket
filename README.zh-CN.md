...
**整套体验 —— 列出、选取即跑，以及手动 `add` / `delete` / `edit` / `find` —— 在每一个 host 上都能用。** 唯一依赖 host 的部分，是*自动*记录那些你重复用过 7 次以上的提示词，因为它必须解析每个 agent 各自的会话格式。目前已经接好了 **Claude Code**、**Codex** 和 **OpenCode**；再加一个平台只是一个 reader 函数的事（见[新增平台](#新增平台)）。在没有扫描器的 host 上，你就用 `add` 自己记录提示词。
...
或者，项目级地，直接打开仓库 —— agent 会读取 `AGENTS.md`。选取即跑和手动 `add/find/edit/delete` 在所有地方都能用；**自动扫描**是唯一依赖 host 的部分（见[新增平台](#新增平台)，了解如何接好一个新 agent 的会话格式）。
...
| 命令 | 作用 |
|---|---|
| `/usually` | 扫描最近的会话，然后**列出**你的高频提示词（每条都带 `id`）。选一条，它立刻就跑。 |
...
1. **浏览并运行** —— 敲 **`/usually:`**（带冒号）。下拉里会为每条保存的提示词填上一项（`/usually:<slug>`）；方向键选中一项，回车运行。
2. **列出并管理** —— 运行 **`/usually`**（直接回车，不带参数）。它会列出你的提示词，让你挑一条来跑。
...
只有当插件的**版本号变化**时，用户才会拿到新版本 —— 否则 `/plugin update` 是个静默的空操作，而且版本号必须在 `.claude-plugin/plugin.json` 和 `.claude-plugin/marketplace.json` **两处**都改。所以每次发布从仓库根目录起就是三步：

```bash
node scripts/bump-version.mjs patch     # 或：minor | major | 0.4.2 （同时更新两个清单）
git commit -am "…" && git push          # 开一个 PR
# 合并到 main
```

就这样 —— 用户用一条 `/plugin update` 就能拿到，他们下次会话的 `SessionStart` hook 会自动重新同步运行时核心（见[更新](#更新)）。

**编辑 skill 文本。** skill 在 `skills/usually/prompt.md`（正文）+ `skills/usually/skill.yaml`（元数据）里只写一次。其他每一份副本 —— 标准的 `skills/usually/SKILL.md` 和每个 host 镜像（`.claude/`、`.opencode/`、`.agents/`、`.gemini/`、`.cursor/`）—— 都是**生成的**。改完源码后，运行：

```bash
node scripts/sync-mirrors.mjs            # 从那对源文件重新生成全部 6 个目标
node scripts/sync-mirrors.mjs --check    # CI/预提交守卫：若有任何目标过期就退出 1
```

`sync-mirrors.test.mjs` 跑的就是同一套 `--check` 逻辑，所以一个过期的镜像会让测试套件失败。

---

## 新增平台

扫描器按每个 agent 读取"仅人类敲入的内容"，再合并计数。每个 agent 有一个小 reader（`claudeTexts` / `codexTexts` / `opencodeTexts`）—— jsonl 或 SQLite，host 用什么就读什么。要支持一个新 agent，照着现有的样子往 `skills/usually/scripts/pocket.mjs` 里加一个 reader：

```js
function myAgentTexts() {
  return readJsonlTexts(join(HOME, '.myagent', 'sessions'), (ev) => {
    if (!isHumanTyped(ev)) return null;     // host 相关的判断
    return cleanCandidate(ev.text);
  });
}
```

然后把它加进 `cmdScan`。其余一切（store、阈值、list、CRUD）都是共享的。每个 agent 的转录格式见 [`skills/usually/reference/store-and-scan.md`](skills/usually/reference/store-and-scan.md)。
